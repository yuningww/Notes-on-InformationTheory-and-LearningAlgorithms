...
\title{Notes on Probabilistic Machine Learning}
\author{}
\date{}
\maketitle
...
\section{Multivariate Probability Models}

\subsection{Measures the dependency}
...
\]
If $\boldsymbol{x} \in \mathbb{R}^D$ is a random vector, its covariance matrix is defined to be the following symmetric, positive semi-definite matrix:
\[
\begin{aligned}
\text{Cov}[\boldsymbol{x}] &\triangleq \mathbb{E} \left[ (\boldsymbol{x} - \mathbb{E}[\boldsymbol{x}])(\boldsymbol{x} - \mathbb{E}[\boldsymbol{x}])^\mathsf{T} \right] \triangleq \boldsymbol{\Sigma} = \boldsymbol{K}_{xx} \\
&= \begin{pmatrix}
...
\end{aligned}
\]
expanding $\mathbb{E} \left[ (\boldsymbol{x} - \mathbb{E}[\boldsymbol{x}])(\boldsymbol{x} - \mathbb{E}[\boldsymbol{x}])^\mathsf{T} \right]$ gives the important result
\[
\boldsymbol{R}_{xx} = \mathbb{E}[\boldsymbol{x} \boldsymbol{x}^\mathsf{T}] = \boldsymbol{\Sigma} + \boldsymbol{\mu} \boldsymbol{\mu}^\mathsf{T}
\]

Under linear transformations, the covariance matrix transforms as follows:
\[
\text{Cov}[\mathbf{A}\boldsymbol{x} + \boldsymbol{b}] = \mathbf{A} \text{Cov}[\boldsymbol{x}] \mathbf{A}^\mathsf{T}
\]

The cross-covariance between two random vectors
\[
\text{Cov}[\boldsymbol{x}, \boldsymbol{y}] = \mathbb{E}[(\boldsymbol{x} - \mathbb{E}[\boldsymbol{x}])(\boldsymbol{y} - \mathbb{E}[\boldsymbol{y}])^\mathsf{T}] = \text{Cov}[\boldsymbol{x}, \boldsymbol{y}]^\top
\]
where $\text{Cov}[\boldsymbol{x}, \boldsymbol{y}] \in \mathbb{R}^{\text{Dim}(\boldsymbol{x}) \times \text{Dim}(\boldsymbol{y})}$ and the $(i, j)$-th entry is $\text{Cov}[\boldsymbol{x}, \boldsymbol{y}]_{ij} = \text{Cov}[\boldsymbol{x}_i, \boldsymbol{y}_j]$

\subsubsection*{Correlation}
The correlation coefficient between X and Y is defined as
\[
\rho \triangleq \text{corr}[X, Y] = \frac{\text{Cov}[X, Y]}{\sqrt{\mathbb{V}[X] \mathbb{V}[Y]}}
\]
which is a normalised measure with $-1 \leq \rho \leq 1$. This can be thought as a measure of \textit{degree of linearity}.
(i.e. corr$[X,Y] = 1$ if and only if $Y = aX+ b$ (and $a>0$) for some parameters $a$ and $b$)

Correlation matrix of a vector $\boldsymbol{x}$ of $D$ related random variables is defined as
\[
\text{corr}(\boldsymbol{x}) = \begin{pmatrix}
...
or compactly as,
\[
\text{corr}(\boldsymbol{x}) = \text{diag}(\boldsymbol{K}_{xx})^{-\frac{1}{2}} \boldsymbol{K}_{xx} \text{diag}(\boldsymbol{K}_{xx})^{-\frac{1}{2}}
\]
where $\boldsymbol{K}_{xx}$ is the (auto)covariance matrix of $\boldsymbol{x}$,
\[
\begin{aligned}
\boldsymbol{K}_{xx} = \boldsymbol{\Sigma} &= \mathbb{E} \left[ (\boldsymbol{x} - \mathbb{E}[\boldsymbol{x}])(\boldsymbol{x} - \mathbb{E}[\boldsymbol{x}])^\mathsf{T} \right] \\
&= \mathbb{E}[\boldsymbol{x} \boldsymbol{x}^\mathsf{T}] - \boldsymbol{\mu} \boldsymbol{\mu}^\mathsf{T} = \boldsymbol{R}_{xx} - \boldsymbol{\mu} \boldsymbol{\mu}^\mathsf{T}
\end{aligned}
\]
where $\boldsymbol{R}_{xx}$ is the (auto)correlation matrix of $\boldsymbol{x}$.

\subsubsection*{Uncorrelated does not imply independent}
Independent implies uncorrelated,
\[
p(X, Y) = p(X)p(Y) \implies \text{Cov}[X, Y] = 0 \implies \text{corr}[X, Y] = 0
\]
However, the converse is not true: uncorrelated does not imply independent (eg. $X \sim U(-1,1)$, $Y = X^2$ have zero correlation but are not independent). 
...
\subsection{The multivariate Gaussian/Normal distribution (MVN)}
...
\]
where $\boldsymbol{\mu} = \mathbb{E}[\boldsymbol{y}] \in \mathbb{R}^D$ is the mean vector, and $\boldsymbol{\Sigma} = \text{Cov}[\boldsymbol{y}]$ is the $D \times D$ covariance matrix.

\subsection{Marginals and conditionals of an MVN}
For a Gaussian random vector $\boldsymbol{y} = (\boldsymbol{y}_1, \boldsymbol{y}_2)$,
if we are conditioned on a set of noise-free observations $\boldsymbol{y}_2$, then we can infer the hidden parts $\boldsymbol{y}_1$ by the posterior $p(\boldsymbol{y}_1 | \boldsymbol{y}_2)$.
...
\boldsymbol{\Sigma} = \begin{pmatrix} \boldsymbol{\Sigma}_{11} & \boldsymbol{\Sigma}_{12} \\ \boldsymbol{\Sigma}_{21} & \boldsymbol{\Sigma}_{22} \end{pmatrix}, \quad
\boldsymbol{\Lambda} = \boldsymbol{\Sigma}^{-1} = \begin{pmatrix} \boldsymbol{\Lambda}_{11} & \boldsymbol{\Lambda}_{12} \\ \boldsymbol{\Lambda}_{21} & \boldsymbol{\Lambda}_{22} \end{pmatrix}
\]
where $\boldsymbol{\Lambda}$ is the \textbf{precision matrix} and $\boldsymbol{\Sigma}_{12} = \boldsymbol{\Sigma}_{21}^\top = \text{Cov}[\boldsymbol{y}_1, \boldsymbol{y}_2]$. 
...
\boxed{
\begin{aligned}
p(\boldsymbol{y}_1 | \boldsymbol{y}_2) &= \mathcal{N}(\boldsymbol{y}_1 | \boldsymbol{\mu}_{1|2}, \boldsymbol{\Sigma}_{1|2}) \\
...
&= \boldsymbol{\Sigma}_{1|2} \left( \boldsymbol{\Lambda}_{11} \boldsymbol{\mu}_1 - \boldsymbol{\Lambda}_{12} (\boldsymbol{y}_2 - \boldsymbol{\mu}_2) \right) \\
...
The proofs rely on computing the Schur complement $\boldsymbol{\Sigma}/\boldsymbol{\Sigma}_{22} = \boldsymbol{\Sigma}_{11} - \boldsymbol{\Sigma}_{12} \boldsymbol{\Sigma}_{22}^{-1} \boldsymbol{\Sigma}_{21}$, which is not shown here.
For the marginals, just extracting the rows and columns corresponding to $\boldsymbol{y}_1$ or $\boldsymbol{y}_2$. 
For the conditional: the conditional mean is a linear function of $\boldsymbol{y}_2$, and the conditional covariance is a constant matrix that is independent of $\boldsymbol{y}_2$. 

\subsection{Linear Gaussian systems and Bayes rule for Gaussians}
...
\]
\[
p(\boldsymbol{y} | \boldsymbol{z}) = \mathcal{N}(\boldsymbol{y} | \mathbf{W}\boldsymbol{z} + \boldsymbol{b}, \boldsymbol{\Sigma}_y)
\]
where $\mathbf{W}$ is a matrix of size $D \times L$. This is an example of a \textbf{linear Gaussian system}.
...
\]
\[
\boldsymbol{\Sigma} = \begin{pmatrix} \boldsymbol{\Sigma}_z & \boldsymbol{\Sigma}_z \mathbf{W}^\mathsf{T} \\ \mathbf{W} \boldsymbol{\Sigma}_z & \boldsymbol{\Sigma}_y + \mathbf{W} \boldsymbol{\Sigma}_z \mathbf{W}^\mathsf{T} \end{pmatrix}
\]
By applying the previous Gaussian conditioning formula to the joint $p(\boldsymbol{y}, \boldsymbol{z})$ we can compute the posterior $p(\boldsymbol{z} | \boldsymbol{y})$ as
\[
\begin{aligned}
p(\boldsymbol{z} | \boldsymbol{y}) &= \mathcal{N}(\boldsymbol{z} | \boldsymbol{\mu}_{z|y}, \boldsymbol{\Sigma}_{z|y}) \\
\boldsymbol{\mu}_{z|y} &= \boldsymbol{\mu}_z + \boldsymbol{\Sigma}_z \mathbf{W}^\mathsf{T} (\boldsymbol{\Sigma}_y + \mathbf{W} \boldsymbol{\Sigma}_z \mathbf{W}^\mathsf{T})^{-1} (\mathbf{y} - \mathbf{W} \boldsymbol{\mu}_z - \mathbf{b}) \\
\boldsymbol{\Sigma}_{z|y} &= \boldsymbol{\Sigma}_z - \boldsymbol{\Sigma}_z \mathbf{W}^\mathsf{T} (\boldsymbol{\Sigma}_y + \mathbf{W} \boldsymbol{\Sigma}_z \mathbf{W}^\mathsf{T})^{-1} \mathbf{W} \boldsymbol{\Sigma}_z
\end{aligned}
\]
However, this requires inverting a $D \times D$ matrix $(\boldsymbol{\Sigma}_y + \mathbf{W} \boldsymbol{\Sigma}_z \mathbf{W}^\mathsf{T})$, which can be expensive if $D$ is large. Instead, we can use the matrix inversion lemma (Woodbury matrix identity) to rewrite the posterior in a more efficient form:
\[
\boxed{
\begin{aligned}
p(\boldsymbol{z} | \boldsymbol{y}) &= \mathcal{N}(\boldsymbol{z} | \boldsymbol{\mu}_{z|y}, \boldsymbol{\Sigma}_{z|y}) \\
\boldsymbol{\Sigma}_{z|y}^{-1} &= \boldsymbol{\Sigma}_z^{-1} + \mathbf{W}^\mathsf{T} \boldsymbol{\Sigma}_y^{-1} \mathbf{W} \\
\boldsymbol{\mu}_{z|y} &= \boldsymbol{\Sigma}_{z|y} \left[ \mathbf{W}^\mathsf{T} \boldsymbol{\Sigma}_y^{-1} (\boldsymbol{y} - \boldsymbol{b}) + \boldsymbol{\Sigma}_z^{-1} \boldsymbol{\mu}_z \right]
\end{aligned}
}
\]
This is known as \textbf{Bayes rule for Gaussians}. Furthermore, the normalization constant of the posterior is given by
\[
p(\boldsymbol{y}) = \int \mathcal{N}(\boldsymbol{z} | \boldsymbol{\mu}_z, \boldsymbol{\Sigma}_z) \mathcal{N}(\boldsymbol{y} | \mathbf{W}\boldsymbol{z} + \boldsymbol{b}, \boldsymbol{\Sigma}_y) \, d\boldsymbol{z} = \mathcal{N}(\boldsymbol{y} | \mathbf{W}\boldsymbol{\mu}_z + \boldsymbol{b}, \boldsymbol{\Sigma}_y + \mathbf{W} \boldsymbol{\Sigma}_z \mathbf{W}^\mathsf{T})
\]

The interpretation is as follows: in the generative story ($\boldsymbol{z} \rightarrow \boldsymbol{y}$), we first sample $\boldsymbol{z}$ from the prior $p(\boldsymbol{z})$, then we sample $\boldsymbol{y}$ from the likelihood $p(\boldsymbol{y} | \boldsymbol{z})$; in the inference story ($\boldsymbol{y} \rightarrow \boldsymbol{z}$), we observe $\boldsymbol{y}$ and want to infer the posterior distribution $p(\boldsymbol{z} | \boldsymbol{y})$.
While the prior and likelihood are specified by the modeler, the posterior is determined by the data $\boldsymbol{y}$ through the Bayes rule.
...
If the optimization goal is to maximize a function $\ell (\boldsymbol{\theta})$ wrt its parameters $\boldsymbol{\theta}$. 
The idea of bound optimization is to construct a surrogate function $Q(\boldsymbol{\theta}; \boldsymbol{\theta}^{t})$ which is a tight lowerbound to $\ell (\boldsymbol{\theta})$ such that:
\[
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.4\linewidth]{images/boundoptimize.png}
  \caption{The solid blue curve is the lower bound, evaluated at $\boldsymbol{\theta}^t$; this
touches the objective function at $\boldsymbol{\theta}^t$. We then set $\boldsymbol{\theta}^{t+1}$ to the maximum of the lower bound (blue
curve), and fit a new bound at that point (dotted green curve). The maximum of this new bound
becomes $\boldsymbol{\theta}^{t+2}$, etc}
\end{figure}

\subsection{EM algorithm}
\textbf{Expectation maximization (EM)} algorithm is a special case of bound optimization 
designed to compute the MLE or MAP parameter estimates for probability models that have \textbf{missing data} and/or \textbf{hidden variables}.

EM algorithm alternate between:
\begin{itemize}
    \item E-step: estimating the hidden variables $\boldsymbol{z}_n$
    \item M-step: using the fully observed data $\boldsymbol{y}_n$ to compute the MLE
\end{itemize}

\subsubsection*{Lower bound: ELBO}
The goal of EM is to maximize the log likelihood of the observed data:
\[
\ell(\boldsymbol{\theta}) = \sum_{n=1}^{N} \log p(\boldsymbol{y}_n \mid \boldsymbol{\theta}) = \sum_{n=1}^{N} \log \left[ \sum_{\boldsymbol{z}_n} p(\boldsymbol{y}_n, \boldsymbol{z}_n \mid \boldsymbol{\theta}) \right]
\]
which is hard to optimize since the log cannot be pushed inside the sum.

Consider a set of arbitrary distribution $q(\boldsymbol{z}_n)$ over each hidden variable $\boldsymbol{z}_n$, then apply Jensen's inequality,
\[
\begin{aligned}
\ell (\boldsymbol{\theta}) = \sum_{n=1}^{N} \log \left[ \sum_{\boldsymbol{z}_n} q(\boldsymbol{z}_n) \frac{p(\boldsymbol{y}_n, \boldsymbol{z}_n \mid \boldsymbol{\theta})}{q(\boldsymbol{z}_n)} \right]
&\geq \sum_{n=1}^{N} \sum_{\boldsymbol{z}_n} q(\boldsymbol{z}_n) \log \frac{p(\boldsymbol{y}_n, \boldsymbol{z}_n \mid \boldsymbol{\theta})}{q(\boldsymbol{z}_n)} \\
&= \sum_n \mathbb{E}_{q_n} \left[ \log p(\boldsymbol{y}_n, \boldsymbol{z}_n | \boldsymbol{\theta}) \right] + \mathbb{H}(q_n) \\
&= \sum_n \text{L}(\boldsymbol{\theta}, q_n) \triangleq \text{L}(\boldsymbol{\theta}, \{q_n\})
\end{aligned}
\]
where $\text{L}(\boldsymbol{\theta}, \{q_n\})$ is the evidence lower bound (ELBO).

\subsubsection*{E step: fix $\boldsymbol{\theta} = \boldsymbol{\theta}^t $, and update $\{q_n\}$}
The lower bound is a sum of $N$ terms, each of which has the form:
\[
\begin{aligned}
\text{L}(\boldsymbol{\theta}, q_n) &= \sum_{\boldsymbol{z}_n} q_n(\boldsymbol{z}_n) \log \frac{p(\boldsymbol{y}_n, \boldsymbol{z}_n | \boldsymbol{\theta})}{q_n(\boldsymbol{z}_n)} \\
&= \sum_{\boldsymbol{z}_n} q_n(\boldsymbol{z}_n) \log \frac{p(\boldsymbol{z}_n | \boldsymbol{y}_n, \boldsymbol{\theta}) p(\boldsymbol{y}_n | \boldsymbol{\theta})}{q_n(\boldsymbol{z}_n)} \\
&= \sum_{\boldsymbol{z}_n} q_n(\boldsymbol{z}_n) \log \frac{p(\boldsymbol{z}_n | \boldsymbol{y}_n, \boldsymbol{\theta})}{q_n(\boldsymbol{z}_n)} + \sum_{\boldsymbol{z}_n} q_n(\boldsymbol{z}_n) \log p(\boldsymbol{y}_n | \boldsymbol{\theta}) \\
&= -D_{\mathbb{KL}} \left( q_n(\boldsymbol{z}_n) \, \| \, p(\boldsymbol{z}_n | \boldsymbol{y}_n, \boldsymbol{\theta}) \right) + \log p(\boldsymbol{y}_n | \boldsymbol{\theta})
\end{aligned}
\]
where $D_{\mathbb{KL}}(q \| p) \geq 0$ and $D_{\mathbb{KL}}(q \| p) = 0$ iff $q = p$.
Hence, the maximization of $\text{L}(\boldsymbol{\theta}, \{q_n\})$ wrt $\{q_n\}$ can be done by setting each one to $q_n^* = p(\boldsymbol{z}_n | \boldsymbol{y}_n, \boldsymbol{\theta})$.
Note that, $p(\boldsymbol{z}_n | \boldsymbol{y}_n, \boldsymbol{\theta}) = \frac{p(\boldsymbol{y}_n, \boldsymbol{z}_n \mid \boldsymbol{\theta})}{\sum_{\boldsymbol{z}'_n} p(\boldsymbol{y}_n, \boldsymbol{z}'_n \mid \boldsymbol{\theta})}$ 
and $p(\boldsymbol{y}_n, \boldsymbol{z}_n \mid \boldsymbol{\theta}) = p(\boldsymbol{y}_n \mid \boldsymbol{z}_n , \boldsymbol{\theta})p(\boldsymbol{z}_n)$ are computable since the prior and conditional distribution on $\boldsymbol{z}_n$ is defined when defining the probabilistic latent model.

This can be shown that fits into the $Q(\boldsymbol{\theta}, \boldsymbol{\theta}^{t})$ formulation.

\subsubsection*{M step: fix $\{q_n\} = \{q_n^*\}$, and update $\boldsymbol{\theta}$}
In the M step, we need to maximize $\text{L}(\boldsymbol{\theta}, \{q_n^t\})$ wrt $\boldsymbol{\theta}$, where the $q_n^t$ are the distributions computed in the E step at iteration $t$. Since the entropy terms $\mathbb{H}(q_n)$ are constant wrt $\boldsymbol{\theta}$, so dropping them in the M step. Then, left with
\[
\ell^t(\boldsymbol{\theta}) = \sum_n \mathbb{E}_{q_n^t} \left[ \log p(\boldsymbol{y}_n, \boldsymbol{z}_n | \boldsymbol{\theta}) \right]
\]
This is the \textbf{expected complete data log likelihood}. Then, the optimization task is,
\[
\boldsymbol{\theta}^{t+1} = \arg\max_{\boldsymbol{\theta}} \sum_n \mathbb{E}_{q_n^t} \left[ \log p(\boldsymbol{y}_n, \boldsymbol{z}_n | \boldsymbol{\theta}) \right]
\]
...
\subsection{EM for a GMM}
\subsubsection*{E step}
The E step computes the responsibility of cluster $k$ for generating data point $n$, as estimated using the current parameter estimates $\boldsymbol{\theta}^{t}$:
\[
r_{nk}^{t} = p^*(z_n = k | \boldsymbol{y}_n, \boldsymbol{\theta}^{t}) = \frac{\pi_k^{t} p(\boldsymbol{y}_n | z_n=k, \boldsymbol{\theta}^{t})}{\sum_{k'} \pi_{k'}^{t} p(\boldsymbol{y}_n | z_n=k', \boldsymbol{\theta}^{t})}
\]
...
\[
\begin{aligned}
\ell^t(\boldsymbol{\theta}) &= \mathbb{E} \left[ \sum_n \log p(z_n | \boldsymbol{\pi}) + \sum_n \log p(\boldsymbol{y}_n | z_n, \boldsymbol{\theta}) \right] \\
&= \mathbb{E} \left[ \sum_n \log \left( \prod_k \pi_k^{z_{nk}} \right) + \sum_n \log \left( \prod_k \mathcal{N}(\boldsymbol{y}_n | \boldsymbol{\mu}_k, \boldsymbol{\Sigma}_k)^{z_{nk}} \right) \right] \\
&= \sum_n \sum_k \mathbb{E}[z_{nk}] \log \pi_k + \sum_n \sum_k \mathbb{E}[z_{nk}] \log \mathcal{N}(\boldsymbol{y}_n | \boldsymbol{\mu}_k, \boldsymbol{\Sigma}_k) \\
&= \sum_n \sum_k r_{nk}^{(t)} \log(\pi_k) - \frac{1}{2} \sum_n \sum_k r_{nk}^{(t)} \left[ \log |\boldsymbol{\Sigma}_k| + (\boldsymbol{y}_n - \boldsymbol{\mu}_k)^\mathsf{T} \boldsymbol{\Sigma}_k^{-1} (\boldsymbol{y}_n - \boldsymbol{\mu}_k) \right] + \text{const}
\end{aligned}
\]
where $z_{nk} = \mathbb{I}(z_n = k)$ is a one-hot encoding of the categorical value $z_n$. Then, the parameter estimates are
\[
\boldsymbol{\mu}_k^{(t+1)} = \frac{\sum_n r_{nk}^{(t)} \boldsymbol{y}_n}{r_k^{(t)}}
\]
\[
\begin{aligned}
\boldsymbol{\Sigma}_k^{(t+1)} &= \frac{\sum_n r_{nk}^{(t)} (\boldsymbol{y}_n - \boldsymbol{\mu}_k^{(t+1)})(\boldsymbol{y}_n - \boldsymbol{\mu}_k^{(t+1)})^\mathsf{T}}{r_k^{(t)}} \\
&= \frac{\sum_n r_{nk}^{(t)} \boldsymbol{y}_n \boldsymbol{y}_n^\mathsf{T}}{r_k^{(t)}} - \boldsymbol{\mu}_k^{(t+1)} (\boldsymbol{\mu}_k^{(t+1)})^\mathsf{T}
\end{aligned}
\]
...
\[
p(y_n \mid \boldsymbol{x}_n, \boldsymbol{w}, \sigma^2)
= \exp\left[\frac{y_n\eta_n - A(\eta_n)}{\sigma^2} + \log h(y_n, \sigma^2)\right]
\]
where 
\begin{itemize}
    \item $y_n$ is the output/response variable, $\boldsymbol{x_n}$ is the input/feature vector, $\boldsymbol{w}$ and $\sigma$ is the learnable model parameter 
    \item $\eta_n \triangleq \boldsymbol{w}^{\mathsf{T}}\boldsymbol{x}_n$ is the (input dependent) natural parameter or canonical parameter
...
\[
\mathbb{E}\left[y_n \mid \boldsymbol{x}_n, \boldsymbol{w}, \sigma^2\right]
...
\[
\mathbb{V}\left[y_n \mid \boldsymbol{x}_n, \boldsymbol{w}, \sigma^2\right]
= A''(\eta_n)\sigma^2
\]
...
\[
p(y_n \mid \boldsymbol{x}_n, \boldsymbol{w})
= \mu_n^{y_n}(1-\mu_n)^{1-y_n}
\]
where
\[
\mu_n = \sigma(\eta_n) = \frac{1}{1 + \exp(-\eta_n)}, \quad
\eta_n = \boldsymbol{w}^{\mathsf{T}}\boldsymbol{x}_n.
\]
Hence
\[
\log p(y_n \mid \boldsymbol{x}_n, \boldsymbol{w})
...
\exp\left(-\frac{1}{2\sigma^2}(y_n - \boldsymbol{w}^{\mathsf{T}}\boldsymbol{x}_n)^2\right)
\]
Hence
\[
\log p(y_n \mid \boldsymbol{x}_n, \boldsymbol{w}, \sigma^2)
= -\frac{1}{2\sigma^2}(y_n - \eta_n)^2 - \frac{1}{2}\log(2\pi\sigma^2)
\]
where $\eta_n = \boldsymbol{w}^{\mathsf{T}}\boldsymbol{x}_n$. In GLM form:
\[
\log p(y_n \mid \boldsymbol{x}_n, \boldsymbol{w}, \sigma^2)
...
\[
\mathbb{E}[y_n] = \eta_n = \boldsymbol{w}^{\mathsf{T}}\boldsymbol{x}_n,\quad \mathbb{V}[y_n] = \sigma^2 
\]
...
\[
p(y_n \mid \boldsymbol{x}_n, N_n, \boldsymbol{w})
= \operatorname{Bin}(y_n \mid \sigma(\boldsymbol{w}^{\mathsf{T}}\boldsymbol{x}_n), N_n)
\]
note that binary logistic regression is the special case when $N_n = 1$. 

The log pdf is given by
\[
\log p(y_n \mid \boldsymbol{x}_n, N_n, \boldsymbol{w})
...
= \log\left[\frac{1}{1+e^{-\boldsymbol{w}^{\mathsf{T}}\boldsymbol{x}_n}}\frac{1+e^{-\boldsymbol{w}^{\mathsf{T}}\boldsymbol{x}_n}}{e^{-\boldsymbol{w}^{\mathsf{T}}\boldsymbol{x}_n}}\right]
= \log\left(\frac{1}{e^{-\boldsymbol{w}^{\mathsf{T}}\boldsymbol{x}_n}}\right)
= \boldsymbol{w}^{\mathsf{T}}\boldsymbol{x}_n
\]
...
\[
p(y_n \mid \boldsymbol{x}_n, \boldsymbol{w})
= \operatorname{Poi}(y_n \mid \exp(\boldsymbol{w}^{\mathsf{T}}\boldsymbol{x}_n))
\]
where
\[
...
\]
where $\mu_n = \exp(\boldsymbol{w}^{\mathsf{T}}\boldsymbol{x}_n)$. Hence in GLM form we have
\[
\log p(y_n \mid \boldsymbol{x}_n, \boldsymbol{w})
= y_n\eta_n - A(\eta_n) + h(y_n)
\]
where $\eta_n = \log(\mu_n) = \boldsymbol{w}^{\mathsf{T}}\boldsymbol{x}_n$, $A(\eta_n) = \mu_n = e^{\eta_n}$, and $h(y_n) = -\log(y_n!)$. Hence
\[
...
\]
where $\eta_n = \boldsymbol{w}^{\mathsf{T}}\boldsymbol{x}_n$. 

To find the MLE, we must solve $\nabla \text{NNL}(\boldsymbol{w}) = \boldsymbol{g(w)} = 0$. For notational simplicity, we will assume $\sigma^2 = 1$. The gradient for a single term is:
\[
\begin{aligned}
\boldsymbol{g}_n &= 
-\frac{1}{\sigma^2}\frac{\partial \ell_n}{\partial \boldsymbol{w}}
=-\frac{1}{\sigma^2} \frac{\partial \ell_n}{\partial \eta_n}\frac{\partial \eta_n}{\partial \boldsymbol{w}} \\
&=-\frac{1}{\sigma^2} \left(y_n - A'(\eta_n)\right)\boldsymbol{x}_n
=-\frac{1}{\sigma^2} (y_n - \mu_n)\boldsymbol{x}_n
\end{aligned}
\]
where $\mu_n = f(\boldsymbol{w^{\mathsf{T}} x})$, and $f$ is the inverse link function that maps from canonical parameters to mean parameters. If we interpret $(y_n - \mu_n)$ as an error signal, then the gradient $-\frac{1}{\sigma^2}\sum_{n=1}^{N}(y_n - \mu_n)\boldsymbol{x}_n$ weights each input $\boldsymbol{x}_n$ by its error, and averages the result.

Gradient-based optimization will find a stationary points where $\boldsymbol{g(w)} = 0$, we need to examine the Hessian matrix to determine whether it is the global optimum. The Hessian is given by 
\[
\begin{aligned}
\boldsymbol{H} &= \frac{\partial^2}{\partial \boldsymbol{w}\partial \boldsymbol{w}^\top}\text{NLL}(\boldsymbol{w}) \\ 
...
\[
\hat{\boldsymbol{w}} \triangleq \operatorname*{argmin}_{\boldsymbol{w}} \mathcal{L}(\boldsymbol{w}) = \operatorname*{argmin}_{\boldsymbol{w}} \text{NLL}({\boldsymbol{w}})
\]
...
\[
\boldsymbol{w}_{t+1}
= \boldsymbol{w}_t - \alpha_t \nabla_{\boldsymbol{w}}\operatorname{NLL}(\boldsymbol{w}_t)
= \boldsymbol{w}_t - \alpha_t(\mu_n - y_n)\boldsymbol{x}_n
\]
...
\textbf{Inference}
Since the encoder gives a \emph{distribution}, not a point:
\begin{itemize}
    \item Option A: use the mean only, $\boldsymbol{z} = \mu_{\boldsymbol{\phi}}(\boldsymbol{x})$, as the reduced representation --- deterministic, ignores $\sigma_{\boldsymbol{\phi}}(\boldsymbol{x})$ entirely. 
...
Kernel methods are a class of nonparametric methods for regression and classification.
Such methods do not assume a fixed parametric form for the prediction function, but instead try to estimate the function itself directly from data.

For a given dataset $\mathcal{D} = \{(\boldsymbol{x}_n, y_n)\}_{n=1}^N$, where $y_n = f(\boldsymbol{x}_n)$ and $f$ is unknown.
To predict the function value at a new point $\boldsymbol{x}_*$, kernel methods predicts that $f(\boldsymbol{x}_*)$ with some weighted combination of the $\{ f(\boldsymbol{x}_n) \}$ values, i.e. remembering the entire
training set $\mathcal{D} = \{(\boldsymbol{x}_n, y_n)\}_{n=1}^N$ to make predictions. 

\section{Mercer Kernels and Mercer's Theorem}

The key to nonparametric methods is that we need a way to encode prior knowlegde about the similarity of two input vectors.
To define similarity, we use a \textbf{kernel function}. 

Here, consider a \textbf{Mercer kernel}, also called a positive definite kernel.
This is any symmetric function $\mathcal{K}: \mathcal{X} \times \mathcal{X} \to \mathbb{R}^+$ such that
\[
\sum_{i=1}^N \sum_{j=1}^N c_i c_j \mathcal{K}(\boldsymbol{x}_i, \boldsymbol{x}_j) \geq 0
\]
for all $\boldsymbol{x}_1, \ldots, \boldsymbol{x}_N \in \mathcal{X}$ which are vectors in $\mathbb{R}^D$ and any choice of numbers $c_i \in \mathbb{R}$. 
Assuming $\mathcal{K}(\boldsymbol{x}_i, \boldsymbol{x}_j) > 0$, so that only equality if $c_i = 0$ for all $i$.
...
\]
then, the condition becomes that $\mathbf{K}$ is positive definite, i.e. all its eigenvalues are positive, for any set of distinct inputs $\{\boldsymbol{x}_i\}_{i=1}^N$.

\subsection{Mercer's theorem}
Any symmetric matrix $\mathbf{K}$ can be represented using an eigendecomposition $\mathbf{K} = \mathbf{U} \boldsymbol{\Lambda} \mathbf{U}^T$ where $\mathbf{U}$ is an orthonormal matrix of eigenvectors and $\boldsymbol{\Lambda}$ is a diagonal matrix of eigenvalues $\lambda_i > 0$.

Consider an element $(i, j)$ of $\mathbf{K}$,
\[
k_{ij} = \mathcal{K}(\boldsymbol{x}_i, \boldsymbol{x}_j)  = (\boldsymbol{\Lambda}^{\frac{1}{2}} \mathbf{U}_{:i})^\top (\boldsymbol{\Lambda}^{\frac{1}{2}} \mathbf{U}_{:j})
\]
where $\mathbf{U}_{:i}$ is the $i$-th column of $\mathbf{U}$. Define a mapping $\boldsymbol{\phi}: \mathbb{R}^D \to \mathbb{R}^N$ as
$\boldsymbol{\phi}(\boldsymbol{x}_i) = \boldsymbol{\Lambda}^{\frac{1}{2}} \mathbf{U}_{:i}$
Then,
\[
k_{ij} = \mathcal{K}(\boldsymbol{x}_i, \boldsymbol{x}_j) = \boldsymbol{\phi}(\boldsymbol{x}_i)^\top \boldsymbol{\phi}(\boldsymbol{x}_j)
= \sum_{m = 1}^N \phi_m(\boldsymbol{x}_i) \phi_m(\boldsymbol{x}_j)
\]
In theory, the kernel function can be generalized to $N \to \infty$ with $\lambda_i > 0$ as a prerequisite.

$\boldsymbol{\phi}$ is a mapping from input space to the feature space, which is a Hilbert space. \textbf{The kernel function $\mathcal{K}$ can be interpreted as an inner product in the feature space}, i.e. $\mathcal{K}(\boldsymbol{x}_i, \boldsymbol{x}_j) = \langle \boldsymbol{\phi}(\boldsymbol{x}_i), \boldsymbol{\phi}(\boldsymbol{x}_j) \rangle$.
The kernel trick is that $\boldsymbol{\phi}$ is never explicitly computed, but from eigendecomposition of $\mathbf{K}$.

\subsection{Mercer kernels examples}

For real-valued inputs $\boldsymbol{x}_i, \boldsymbol{x}_j \in \mathbb{R}^D$, it is common to use stationary kernels, which are functions of the form $\mathcal{K}(\boldsymbol{x}_i, \boldsymbol{x}_j) = \mathcal{K}(\|\boldsymbol{x}_i - \boldsymbol{x}_j\|)$, i.e. the value depends only on the elementwise distance between the two inputs.

\subsubsection*{Gaussian Kernel}
Also called the squared exponential kernel, exponentiated quadratic kernel, RBF kernel, defined as
\[
\mathcal{K}(\boldsymbol{x}, \boldsymbol{x}')= \exp\left(-\frac{\|\boldsymbol{x} - \boldsymbol{x}'\|^2}{2\ell^2}\right)
\]
$\ell$ is the length scale of the kernel, i.e., the distance over which we expect differences to matter. This is known as the bandwidth parameter.

\subsubsection*{ARD Kernel}
Automatic relevancy determination kernel is a generalization of the Gaussian kernel by replacing Euclidean distance with Mahalanobis distance, defined as
\[
\mathcal{K}(\boldsymbol{x}, \boldsymbol{x}') = \sigma^2 \exp \left( -\frac{1}{2} \boldsymbol{r}^\top \boldsymbol{\Sigma}^{-1} \boldsymbol{r} \right)
\]

Typically, $\boldsymbol{\Sigma}$ is a diagonal matrix with $\ell_i^2$ on the diagonal, thus
\[
\mathcal{K}(\boldsymbol{x}, \boldsymbol{x}') = \sigma^2 \exp \left( - \sum_{i=1}^D \frac{(x_i - x'_i)^2}{2 \ell_i^2} \right)
\]
which allows each input dimension $i$ to have its own characteristic length scale. For high-dimensional inputs where some dimensions may be more relevant than others (eg. $\ell_i = \infty$ makes that dimension been ignored).
...
This is a way to get a conjunction of the individual properties of each kernel.
\[
\mathcal{K}(\boldsymbol{x}, \boldsymbol{x}') = 
\mathcal{K}_1(\boldsymbol{x}, \boldsymbol{x}') \times \mathcal{K}_2(\boldsymbol{x}, \boldsymbol{x}')
\]
...
\[
\mathcal{K}(\boldsymbol{x}, \boldsymbol{x}') = 
\mathcal{K}_1(\boldsymbol{x}, \boldsymbol{x}') + \mathcal{K}_2(\boldsymbol{x}, \boldsymbol{x}')
\]
\clearpage
\section{Gaussian Processes}
\textbf{Gaussian processes} is a way to define distributions over functions of the form $f: \mathcal{X} \to \mathbb{R}$, where $\mathcal{X}$ is any domain. 
The key assumption is that the function values at a set of $M > 0$ inputs, $\boldsymbol{f} = [f(\boldsymbol{x}_1), \ldots, f(\boldsymbol{x}_M)]$, is jointly Gaussian, with mean $(\boldsymbol{\mu} = m(\boldsymbol{x}_1), \ldots, m(\boldsymbol{x}_M))$ and covariance $\boldsymbol{\Sigma}_{ij} = \mathcal{K}(\boldsymbol{x}_i, \boldsymbol{x}_j)$, where $m$ is a mean function and $\mathcal{K}$ is a positive definite (Mercer) kernel. 

Since we assume this holds for any $M > 0$, this includes the case where $M = N+1$, containing $N$ training points $\boldsymbol{x}_n$ and 1 test point $\boldsymbol{x}_*$. 
Then, $f(\boldsymbol{x}_*)$ can be inferred from knowledge of $f(\boldsymbol{x}_1), \ldots, f(\boldsymbol{x}_n)$ by manipulating the joint Gaussian distribution $p(f(\boldsymbol{x}_1), \ldots, f(\boldsymbol{x}_N), f(\boldsymbol{x}_*))$. Also, extending this to noisy functions of $f(\boldsymbol{x}_n)$ can deal with problems such as regression or classification.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.4\linewidth]{images/GPdemo.png}
  \caption{A Gaussian process for 2 training points and 1 test point. The hidden nodes $f_i = f(\boldsymbol{x}_i)$ represent the value of the function at each of the data points.
  These hidden nodes are fully interconnected with the edge represent the covaiance terms $\Sigma_{ij} = \mathcal{K}(\boldsymbol{x}_i, \boldsymbol{x}_j)$.
  If the testing points $\boldsymbol{x}_*$ is close to the training points, then the predicted function value $f(\boldsymbol{x}_*)$ will be similar to the training points $f(\boldsymbol{x}_1)$ and $f(\boldsymbol{x}_2)$.}
\end{figure}

\subsection{Noise-free observations}
Suppose we observe a training set $\mathcal{D} = \{(\boldsymbol{x}_n, y_n) : n = 1:N\}$, where $y_n = f(\boldsymbol{x}_n)$ is the noise-free observation of the function evaluated at $\boldsymbol{x}_n$. 
If we ask the GP to predict $f(\boldsymbol{x})$ for a value of $\boldsymbol{x}$ that it has already seen, we want the GP to return the answer $f(\boldsymbol{x})$ with no uncertainty. 

Now for the case of predicting the outputs for new inputs. 
Specifically, given a test set $\mathbf{X}_*$ with $N_*$ new data points, we want to predict the function outputs $\boldsymbol{f}_* = [f(\boldsymbol{x}_{*1}), \ldots, f(\boldsymbol{x}_{*N_*})]$. By definition of the GP, the joint distribution $p(\boldsymbol{f}_X, \boldsymbol{f}_* | \mathbf{X}, \mathbf{X}_*)$ has the following form
\[
\begin{pmatrix} \boldsymbol{f}_X \\ \boldsymbol{f}_* \end{pmatrix} \sim \mathcal{N} \left( \begin{pmatrix} \boldsymbol{\mu}_X \\ \boldsymbol{\mu}_* \end{pmatrix}, \begin{pmatrix} \mathbf{K}_{X,X} & \mathbf{K}_{X,*} \\ \mathbf{K}_{X,*}^\mathsf{T} & \mathbf{K}_{*,*} \end{pmatrix} \right)
\]
where $\boldsymbol{\mu}_X = [m(\boldsymbol{x}_1), \ldots, m(\boldsymbol{x}_N)]$, $\boldsymbol{\mu}_* = [m(\boldsymbol{x}_{*1}), \ldots, m(\boldsymbol{x}_{*N_*})]$, $\mathbf{K}_{X,X} = \mathcal{K}(\mathbf{X}, \mathbf{X})$ is $N \times N$, $\mathbf{K}_{X,*} = \mathcal{K}(\mathbf{X}, \mathbf{X}_*)$ is $N \times N_*$, and $\mathbf{K}_{*,*} = \mathcal{K}(\mathbf{X}_*, \mathbf{X}_*)$ is $N_* \times N_*$. 
By the standard rules for conditioning Gaussians, the posterior has the following form
\[
\begin{aligned}
p(\boldsymbol{f}_* | \mathbf{X}_*, \mathcal{D}) &= \mathcal{N}(\boldsymbol{f}_* | \boldsymbol{\mu}_{*|X}, \boldsymbol{\Sigma}_{*|X}) \\
\boldsymbol{\mu}_{*|X} &= \boldsymbol{\mu}_* + \mathbf{K}_{X,*}^\mathsf{T} \mathbf{K}_{X,X}^{-1} (\boldsymbol{f}_X - \boldsymbol{\mu}_X) \\
\boldsymbol{\Sigma}_{*|X} &= \mathbf{K}_{*,*} - \mathbf{K}_{X,*}^\mathsf{T} \mathbf{K}_{X,X}^{-1} \mathbf{K}_{X,*}
\end{aligned}
\]

\subsection{Noisy observations}
Suppose we observe a training set $\mathcal{D} = \{(\boldsymbol{x}_n, y_n) : n = 1:N\}$, where $y_n = f(\boldsymbol{x}_n) + \epsilon_n$ is the noisy observation of the function evaluated at $\boldsymbol{x}_n$, and $\epsilon_n \sim \mathcal{N}(0, \sigma_y^2)$ is i.i.d. Gaussian noise.
Consider, the covariance of the observed noisy responses is 
\[
\begin{aligned}
\text{Cov}[y_i, y_j] &= \text{Cov}[f(\boldsymbol{x}_i) + \epsilon_i, f(\boldsymbol{x}_j) + \epsilon_j] \\
&= \text{Cov}[f(\boldsymbol{x}_i), f(\boldsymbol{x}_j)] + \text{Cov}[\epsilon_i, \epsilon_j] \\
&= \mathcal{K}(\boldsymbol{x}_i, \boldsymbol{x}_j) + \sigma_y^2 \delta_{ij}
\end{aligned}
\]
Writing them compactly as,
\[
\text{Cov}[\boldsymbol{y} \mid \mathbf{X}] = \mathbf{K}_{X,X} + \sigma_y^2 \mathbf{I}_N \triangleq \mathbf{K}_{\sigma}
\]

The joint density of the observed data and the latent function on the test points is given by
\[
\begin{pmatrix} \boldsymbol{y} \\ \boldsymbol{f}_* \end{pmatrix} \sim \mathcal{N} \left( \begin{pmatrix} \boldsymbol{\mu}_X \\ \boldsymbol{\mu}_* \end{pmatrix}, \begin{pmatrix} \mathbf{K}_{\sigma} & \mathbf{K}_{X,*} \\ \mathbf{K}_{X,*}^\mathsf{T} & \mathbf{K}_{*,*} \end{pmatrix} \right)
\]
Therefore, on a set of test points $\mathbf{X}_*$, the posterior predictive distribution is
\[
\begin{aligned}
p(\boldsymbol{f}_* | \mathbf{X}_*, \mathcal{D}) &= \mathcal{N}(\boldsymbol{f}_* | \boldsymbol{\mu}_{*|X}, \boldsymbol{\Sigma}_{*|X}) \\
\boldsymbol{\mu}_{*|X} &= \boldsymbol{\mu}_* + \mathbf{K}_{X,*}^\mathsf{T} \mathbf{K}_{\sigma}^{-1} (\boldsymbol{f}_X - \boldsymbol{\mu}_X) \\
\boldsymbol{\Sigma}_{*|X} &= \mathbf{K}_{*,*} - \mathbf{K}_{X,*}^\mathsf{T} \mathbf{K}_{\sigma}^{-1} \mathbf{K}_{X,*}
\end{aligned}
\]

\subsection{Bayesian linear regression}
Bayesian linear regression is a special case of Gaussian processes, and can be viewed in weight space or function space.
In the following example, lets consider the case where we have $N$ training points $\mathcal{D} = \{ (\boldsymbol{x}_n, \boldsymbol{y}_n) : n = 1 : N \}$ and $1$ testing points $\boldsymbol{x}_*$.

\subsubsection*{Weight space view}
Consider the linear regression model $y = f(\boldsymbol{x}) + \epsilon$, where $f(\boldsymbol{x}) = \boldsymbol{w}^\mathsf{T} \boldsymbol{\phi}(\boldsymbol{x})$ and $\epsilon \sim \mathcal{N}(0, \sigma_y^2)$. With vectorized form:
\[
\boldsymbol{y} = \boldsymbol{\Phi} \boldsymbol{w} + \boldsymbol{\epsilon} \implies p(\boldsymbol{y} | \boldsymbol{w}) = \mathcal{N}(\boldsymbol{y} \mid \boldsymbol{\Phi} \boldsymbol{w}, \sigma_y^2 \mathbf{I})
\]
where $\boldsymbol{\Phi}$ is the $N \times D$ design matrix including features of all $N$ training points.
Plain linear regression stops by maximizing this w.r.t. $\boldsymbol{w}$, but the Bayesian pushes this forward.

Assuming a Gaussian prior $p(\boldsymbol{w}) = \mathcal{N}(\boldsymbol{w} | \boldsymbol{0}, \boldsymbol{\Sigma}_w)$ on the model parameter $\boldsymbol{w} \in \mathbb{R}^{D \times 1}$, then the posterior is as follows:
\[
p(\boldsymbol{w} | \mathcal{D}) = \mathcal{N}\left(\boldsymbol{w} \Big| \frac{1}{\sigma_y^2} \mathbf{A}^{-1} \boldsymbol{\Phi}^T \boldsymbol{y}, \, \mathbf{A}^{-1}\right) \propto p(\boldsymbol{y} | \boldsymbol{w}) p(\boldsymbol{w}) 
\]
which is derived by completing the square with $\mathbf{A} = \sigma_y^{-2} \boldsymbol{\Phi}^\mathsf{T} \boldsymbol{\Phi} + \boldsymbol{\Sigma}_w^{-1}$.

The posterior predictive distribution for $f_* = f(\boldsymbol{x}_*) = \boldsymbol{\phi}_*^\mathsf{T} \boldsymbol{w} = \boldsymbol{\phi}(\boldsymbol{x}_*)^\mathsf{T} \boldsymbol{w}$ is therefore
\[
p(f_* | \mathcal{D}, \boldsymbol{x}_*) = \mathcal{N}\left(f_* \Big| \frac{1}{\sigma_y^2} \boldsymbol{\phi}_*^\mathsf{T} \mathbf{A}^{-1} \boldsymbol{\Phi}^\mathsf{T} \boldsymbol{y}, \; \boldsymbol{\phi}_*^\mathsf{T} \mathbf{A}^{-1} \boldsymbol{\phi}_*\right)
\]
using the standard rules for linear transforms of Gaussians ($\boldsymbol{z} = \boldsymbol{c}^\mathsf{T} \boldsymbol{w} \implies \mathbb{E}[z] = \boldsymbol{c}^\mathsf{T} \boldsymbol{\mu}_w$, $\text{Cov}(z) = \boldsymbol{c}^\mathsf{T} \boldsymbol{\Sigma}_{w} \boldsymbol{c}$).

\subsubsection*{Function space view}
Bayesian linear regression can be viewed as a GP with a kernel $\mathcal{K}(\boldsymbol{x}, \boldsymbol{x}') = \boldsymbol{\phi}(\boldsymbol{x})^\mathsf{T} \boldsymbol{\Sigma}_w \boldsymbol{\phi}(\boldsymbol{x}')$, which measures the similarity between two inputs $\boldsymbol{x}$ and $\boldsymbol{x}'$ in the feature space defined by $\boldsymbol{\phi}$ and is also a valid
kernel since covariance $\boldsymbol{\Sigma}_w$ is always positive semi-definite.

Then, defining the training-training, training-test, and test-test covariance:
\[
\mathbf{K} = \boldsymbol{\Phi}^\top \boldsymbol{\Sigma}_w \boldsymbol{\Phi} \in \mathbb{R}^{N \times N}, \quad
\mathbf{k}_* = \boldsymbol{\Phi}^\top \boldsymbol{\Sigma}_w \boldsymbol{\phi}_* \in \mathbb{R}^{N \times 1}, \quad
k_{**} = \boldsymbol{\phi}_*^\top \boldsymbol{\Sigma}_w \boldsymbol{\phi}_* \in \mathbb{R}
\]
where $\boldsymbol{\phi}_*$ is a $D \times 1$ is the feature vector of the single test point.
Using these notation, and the matrix inversion lemma, we can rewrite the predictive posterior in the weight space view section as:
\[
\begin{aligned}
p(f_* | \mathcal{D}, \boldsymbol{x}_*) &= \mathcal{N}(f_* | \boldsymbol{\mu}_{*|X}, \boldsymbol{\Sigma}_{*|X}) \\
\boldsymbol{\mu}_{*|X} &= \boldsymbol{\phi}_*^\mathsf{T} \boldsymbol{\Sigma}_w \boldsymbol{\Phi}^\mathsf{T} (\mathbf{K} + \sigma_y^2 \mathbf{I})^{-1} \boldsymbol{y} = \boldsymbol{k}_*^\mathsf{T} \mathbf{K}_\sigma^{-1} \boldsymbol{y} \\
\boldsymbol{\Sigma}_{*|X} &= \boldsymbol{\phi}_*^\mathsf{T} \boldsymbol{\Sigma}_w \boldsymbol{\phi}_* - \boldsymbol{\phi}_*^\mathsf{T} \boldsymbol{\Sigma}_w \boldsymbol{\Phi}^\mathsf{T} (\mathbf{K} + \sigma_y^2 \mathbf{I})^{-1} \boldsymbol{\Phi} \boldsymbol{\Sigma}_w \boldsymbol{\phi}_* = k_{**} - \boldsymbol{k}_*^\mathsf{T} \mathbf{K}_\sigma^{-1} \boldsymbol{k}_*
\end{aligned}
\]
which matches the predictive posterior in the GP with noisy observations section with only one testing point $\boldsymbol{x}_*$ instead of $N_*$ testing points $\mathbf{X}_*$.

With kernel trick and a valid kernel $\mathcal{K}$, we can compute the predictive posterior without explicitly computing the feature mapping $\boldsymbol{\phi}$ and allowing for a infinite length feature space.


\subsection{Estimating the kernel}
Most kernels have some free parameters, which can have a large effect on the predictions from the model.
To estimate the kernel parameters $\boldsymbol{\theta}$ (i.e. hyperparameters), we could use exhaustive search over a discrete grid of values,
with validation loss as an objective, but this can be quite slow.

\subsubsection*{Empirical Bayes}
Consider an empirical Bayes approach, which allows the use of gradient-based optimization methods, which are much faster. In particular, we will maximize the marginal likelihood
\[
...
\]
The latent Gaussian vector $\boldsymbol{f}$ have been marginalized out, so this is called the marginal likelihood rather than likelihood.

Recall that the prior $p(\boldsymbol{f} | \boldsymbol{X}) = \mathcal{N}(\boldsymbol{f} | \boldsymbol{0}, \boldsymbol{K}_{X,X})$ and the likelihood $p(\boldsymbol{y} | \boldsymbol{f}) = \mathcal{N}(\boldsymbol{y} | \boldsymbol{f}, \sigma_y^2 \mathbf{I}_N)$, in which we assume the mean function is 0, then the marginal likelihood is given by
\[
\log p(\boldsymbol{y} | \mathbf{X}, \boldsymbol{\theta}) = \log \mathcal{N}(\boldsymbol{y} | \boldsymbol{0}, \mathbf{K}_\sigma) = -\frac{1}{2} \boldsymbol{y}^\mathsf{T} \mathbf{K}_\sigma^{-1} \boldsymbol{y} - \frac{1}{2} \log |\mathbf{K}_\sigma| - \frac{N}{2} \log(2\pi)
\]
where the dependence of $\mathbf{K}_\sigma = \mathbf{K}_{X,X} + \sigma_y^2 \mathbf{I}_N$ on $\boldsymbol{\theta}$ is implicit.
The first term is a data fit term, the second term is a model complexity term, and the third term is a constant.
There is a tradeoff between the first and the second term.

To maximize the marginal likelihood,
\[
\begin{aligned}
\frac{\partial}{\partial \theta_j} \log p(\boldsymbol{y} | \mathbf{X}, \boldsymbol{\theta}) &= \frac{1}{2} \boldsymbol{y}^\mathsf{T} \mathbf{K}_\sigma^{-1} \frac{\partial \mathbf{K}_\sigma}{\partial \theta_j} \mathbf{K}_\sigma^{-1} \boldsymbol{y} - \frac{1}{2} \text{tr}\left(\mathbf{K}_\sigma^{-1} \frac{\partial \mathbf{K}_\sigma}{\partial \theta_j}\right) \\
&= \frac{1}{2} \text{tr}\left( (\boldsymbol{\alpha}\boldsymbol{\alpha}^\mathsf{T} - \mathbf{K}_\sigma^{-1}) \frac{\partial \mathbf{K}_\sigma}{\partial \theta_j} \right)
\end{aligned}
\]
where $\boldsymbol{\alpha} = \mathbf{K}_\sigma^{-1} \boldsymbol{y}$. It takes $O(N^3)$ time to compute $\mathbf{K}_\sigma^{-1}$, and then $O(N^2)$ time per hyper-parameter to compute the gradient.
Therefore, we can estimate the kernel parameters using any standard gradient-based optimizer. However, since the objective is not convex, local minima can be a problem, so multiple restarts might be used.
