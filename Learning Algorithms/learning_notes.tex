...
The mapping $\boldsymbol{\phi}$ is a mapping from input space to the feature space, which is a Hilbert space. \textbf{The kernel function $\mathcal{K}$ can be interpreted as an inner product in the feature space}, i.e. $\mathcal{K}(\boldsymbol{x}_i, \boldsymbol{x}_j) = \langle \boldsymbol{\phi}(\boldsymbol{x}_i), \boldsymbol{\phi}(\boldsymbol{x}_j) \rangle$.
